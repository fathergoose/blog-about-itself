...
function createLinkElements(articles: any[]) {
  return articles.map(
    (a: { webPath: any; title: any; previewText: any }) =>
...
function buildArticles(template: string) {
...
function getTitle(filePath: PathOrFileDescriptor): string {
...
function buildPreviewText(filePath: PathOrFileDescriptor) {
  const decoder = new TextDecoder();
  const data = readFileSync(filePath);
  const text = decoder.decode(data) ?? "";
...
try {
  ...
  const articleSummeries = htmlFilePaths.map((filePath) => ({
    filePath,
    webPath: filePath.slice(5), // remove "./www"
